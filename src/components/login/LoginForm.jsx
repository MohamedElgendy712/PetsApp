        ...
        authenticate(email, password).then((response) => {
            ...
            if (response.status === "error") {
                if (response.type === "validation") {
                    setEmailErrorMessage(response.data.email !== undefined ? response.data.email : "")
                    setPasswordErrorMessage(response.data.password !== undefined ? response.data.password : "")
                }
    ...
    return (
        <form className={props.class} method="POST">
            ...
            <div className="mb-3 text-right">
                <a className="link-plain text-grey bold" href="{{url('/admin/password-reset')}}">Forgot Password ?</a>
            </div>

            <button disabled={isLoggingIn} type="submit" className="btn btn-primary w-100" onClick={(e) => {login(e)}}>Login</button>
        </form>
    )
}
